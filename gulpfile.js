...
async function translationCheckTask() {
  await gulp.src(roptions.inputs.source, { allowEmpty: true })
...
const resources = resourceSpecs.reduce((acc, spec) => {
  ...
  const copyTask = () => {
    return gulp.src(spec.source, { allowEmpty: true })
...
async function mochaTask() {
  await gulp.src(roptions.outputs.test.file, { allowEmpty: true })
    .on("error", (err) => {
...
async function lintTaskCore() {
  gulp.src(roptions.inputs.lint, { allowEmpty: true })
...
async function lintTask18() {
  gulp.src(roptions.inputs.lint18, { allowEmpty: true })
