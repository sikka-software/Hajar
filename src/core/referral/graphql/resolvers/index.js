...
export async function createReferral(args) {
    const referral = await ReferralModels.Referral.findOne({ code: args.referralInput.code });
...
export async function updateReferral(args) {
    ...
    const newReferral = await ReferralModels.Referral.findByIdAndUpdate(args.referralId, newReferralData, {
        new: true
    });
    return { ...newReferral._doc };
};
...
export async function createReferralAnalytics(args) {
    const referral = await ReferralModels.Referral.findOne({ code: args.referralAnalyticsInput.referral });
    if (!referral) throw new Error("Referral does not exist");
...
export async function updateReferralAnalytics(args) {
    const referralAnalytics = await ReferralModels.ReferralAnalytics.findById(args.referralAnalyticsId);
    ...
    const newReferralAnalytics = await ReferralModels.ReferralAnalytics.findByIdAndUpdate(args.referralAnalyticsId, newReferralAnalyticsData, {
        new: true
    });
    return { ...newReferralAnalytics._doc };
};
...
export async function deleteReferralAnalytics(args) {
    ...
    const deleteReferralAnalytics = await ReferralModels.ReferralAnalytics.deleteOne({
        _id: args.referralAnalyticsId
    });
};

export async function referralAnalytics(args) {
    let referralAnalytics = await ReferralModels.ReferralAnalytics.findById(args.referralAnalyticsId);
    if (!referralAnalytics) throw new Error("Could not find any referral analytics");
...
export async function referralsAnalytics(args) {
    let allReferralAnalytics = await ReferralModels.ReferralAnalytics.Referral.find();
